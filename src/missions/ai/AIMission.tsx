...
export function AIMission({ mission, progress, onProgress, onBack }: Props) {
  ...
  return <section className="ai-mission ai-guided panel panel--wide">
    <div className="component-mission__topline"><button className="back-link" onClick={onBack}>← Retour à la carte</button>{!complete && <div className="ai-top-actions"><div className="ai-context-help"><button className="button button--hint" onClick={getHelp}>Besoin d’aide ?</button>{showHelp && helpCount > 0 && <div role="status"><b>Coup de pouce {helpCount}/3</b><p>{guide.help[Math.min(helpCount, 3) - 1]}</p><button className="ai-help-close" onClick={() => setShowHelp(false)}>Fermer l’aide</button></div>}</div><button className="button button--quiet" onClick={reset}>↺ Réinitialiser la mission IA</button></div>}</div>
    <header className="ai-guided-header"><p className="eyebrow">Module 6 · Intelligence artificielle</p><h1>Apprendre à la machine à observer le ciel</h1><div className="ai-step-indicator" aria-label={`Étape ${step} sur 4`}>{([1, 2, 3, 4] as const).map((number) => <span key={number} className={number === step ? 'is-current' : number < step ? 'is-done' : ''}>{number === step ? `Étape ${number} sur 4 — ${stepCopy[level][number].title}` : `${number < step ? '✓' : number} ${stepCopy[level][number].title}`}</span>)}</div></header>
    {!complete && <><div className="ai-guide"><span className="ai-guide__seal" aria-hidden="true">A</span><div><strong>Le guide de l’observatoire</strong><h2>{guide.title}</h2><p>{guide.guide}</p></div></div><div className="ai-task"><b>À toi de jouer</b><p>{taskInstruction}</p></div></>}
    ...
    {complete && <div className="ai-success"><div className="ai-six" aria-label="Fragment découvert : 6"><span>{newObservations.map((item) => <i key={item.id}>{item.name}</i>)}</span><strong>6</strong></div><div><p className="eyebrow">Fragment 6 enregistré automatiquement · module IA allumé</p><h2>Les observations révèlent le 6</h2><p>O1 et O2 : signal. O3 et O4 : parasite.</p><p>Ici, nous simulons une petite IA. Les vraies IA peuvent utiliser beaucoup plus d’exemples et de calculs, mais le principe reste le même : apprendre à partir de données.</p><p>Une IA peut se tromper. Pour bien l’utiliser, il faut vérifier ses données et garder un regard humain.</p><button className="button button--primary" onClick={onBack}>Retourner à la carte →</button></div></div>}
