...
export function MemoryMission({ mission, progress, onProgress, onBack }: Props) {
  ...
  return <section className="memory-mission panel panel--wide">
    <div className="component-mission__topline"><button className="back-link" onClick={onBack}>← Retour au laboratoire</button>{!game.complete && <button className="button button--quiet" onClick={reset}>↺ Réinitialiser le classement</button>}</div>
    <MissionHeader mission={mission}><p aria-label={`${placed}/${cards.length} cartes classées`}><b>{placed}/{cards.length}</b> cartes classées</p></MissionHeader>
    <InstructionPanel mood={game.feedback.kind === 'error' ? 'error' : 'explain'}><p>Touche une carte puis une zone, ou fais-la glisser.</p></InstructionPanel><div className="memory-room">
    ...
    {game.complete&&<CompletionPanel mission={mission} title="La bibliothèque révèle son secret" onBack={onBack}>{mission.learning}</CompletionPanel>}
  </section>;
}
